...
namespace AngryMonkey
{
    ...
    {
        public static Dictionary<string, string> GetFrontMatter(string[] lines)
        {
        ...
        public static string ReplaceFrontMatter(string[] lines, Dictionary<string, string> yaml)
        {
        ...
        public static string EnsureYamlSlug(string markdown, string title, bool replaceExisting = true)
        {
            if (markdown is null) throw new ArgumentNullException(nameof(markdown));
            title ??= "";
            ...
            if (looksLikeYamlStart)
            {
                ...
                if (end != -1)
                {
                    ...
                    //{
                    //    if (replaceExisting)
                    //        yaml[titleLine] = $"uid: {title}";
                    //}
                    //else
                    {
                        yaml = yaml.Concat([$"uid: {title}"]).ToArray();
                    }
